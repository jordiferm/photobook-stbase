...
STGraphicsItemModifier::STGraphicsItemModifier(QGraphicsItem* _Item) : Item(_Item)
	, m_xRotationAngle(0)
	, m_yRotationAngle(0)
	, m_zRotationAngle(0)
	, CScaleX(1)
...
QRect STGraphicsItemModifier::contentsRect() const
{
    return m_contentsRect;
}

void STGraphicsItemModifier::updateToolTip()
{
	updateToolTip(QRectF(Item->pos(), Item->transform().mapRect(Item->boundingRect()).size()));
}

void STGraphicsItemModifier::updateToolTip(const QRectF& _Rect)
{
...
void STGraphicsItemModifier::resetContentsRatio()
{
    resizeContents(m_contentsRect, true);
}

void STGraphicsItemModifier::scale(double _Sx, double _Sy)
{
	CScaleX = _Sx;
	CScaleY = _Sy;
...
void STGraphicsItemModifier::setRectBottomRight(const QPointF& _Pos, QGraphicsItem* _Sender)
{
	if (STGraphicsPhotoItem* CItem = qgraphicsitem_cast<STGraphicsPhotoItem*>(Item))
	{
		QPointF Pos = _Sender->mapToScene(_Pos); 
...
void STGraphicsItemModifier::resizeContents(const QRect & rect, bool keepRatio)
{
...
void STGraphicsItemModifier::layoutChildren()
{
	...
	{		
		if (STCornerItem* CItem = qgraphicsitem_cast<STCornerItem*>(*it))
			CItem->relayout(Item->boundingRect().toRect());
	}
}

void STGraphicsItemModifier::rotate(double _Angle)
{
...
void STGraphicsItemModifier::setChildrenVisible(bool _Value)
{
	QList<QGraphicsItem *> Items = Item->childItems(); 
	QList<QGraphicsItem *>::iterator it; 
	for (it = Items.begin(); it != Items.end(); ++it)
