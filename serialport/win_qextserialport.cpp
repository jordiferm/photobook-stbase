...
*/
qlonglong Win_QextSerialPort::size() const {
    int availBytes;
    COMSTAT Win_ComStat;
...
void Win_QextSerialPort::translateError(unsigned long error) {
	...
	/*!
	\fn Q_UINT64_C Win_QextSerialPort::readBlock(char *data, uint maxlen)
	...
	associated with the class is not currently open.
	*/
	quint64 Win_QextSerialPort::readBlock(char *data, 
	#ifdef QTVER_PRE_30                                     
													uint maxlen) 
	#else
...
#if QT_VERSION < 0x040000
	/*!
	\fn Q_UINT64_C Win_QextSerialPort::writeBlock(const char *data, uint len)
	...
	*/
	quint64 Win_QextSerialPort::writeBlock(const char *data, 
	#ifdef QTVER_PRE_30
													uint len) 
	#else
													unsigned long len)
...
{
	return const_cast<Win_QextSerialPort*>(this)->bytesWaiting();
}

bool Win_QextSerialPort::waitForReadyRead ( int _MSecs)
{
