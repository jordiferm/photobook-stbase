...
bool Posix_QextSerialPort::open(int) {
    ...
    if (!portOpen) {
        ...
        /*open the port*/
				...
				Posix_File->setFileName(portName);
			#else
				Posix_File->setName(portName);	
			#endif
		  
        //if (Posix_File->open(IO_Async|IO_Raw|QIODevice::ReadWrite)) 
		  //! Compte que no se si open obrirà de la mateixa forma.
		  if (Posix_File->open(QIODevice::ReadWrite)) 
		  {
				//Obrim el dispositiu de la base amb el mode ReadWrite.
...
*/
qlonglong Posix_QextSerialPort::size() const {
    int availBytes;
...
bool Posix_QextSerialPort::waitForReadyRead ( int _MSecs)
{
	setTimeout(_MSecs / 1000, _MSecs % 1000);
	return (bytesWaiting() > 0);
}

qint64 Posix_QextSerialPort::bytesAvailable () const
{
	return const_cast<Posix_QextSerialPort*>(this)->bytesWaiting();
}
...
{
	//qDebug("CRIDAT: qint64 Posix_QextSerialPort::writeData ( const char * data, qint64 maxSize )");
	LOCK_MUTEX();
	int retVal=0;
	if (portOpen) {
