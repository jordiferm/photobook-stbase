...
bool DMetadata::setImageComment(const QString& comment) const
{
    ...
    qDebug() << getFilePath() << " ==> Comment: " << comment << endl;
...
bool DMetadata::setImageRating(int rating) const
{
    ...
    if (rating < RatingMin || rating > RatingMax)
    {
        qDebug() << "Rating value to write is out of range!" << endl;
        return false;
    }

    qDebug() << getFilePath() << " ==> Rating: " << rating << endl;
                           ...
                           const char* debugLabel, const char* tagKey)  const
{
    ...
    qDebug() << getFilePath() << " ==> " << debugLabel << ": " << truncatedText << endl;
...
QString DMetadata::valueToString (const QVariant &value, MetadataInfo::Field field)
{
    ...
    switch (field)
    {
        ...
        case MetadataInfo::Orientation:
            switch (value.toInt())
            {
                ...
                case ORIENTATION_NORMAL:
                    return QObject::tr("Rotation of an unrotated image", "Normal");
                case ORIENTATION_HFLIP:
                    return QObject::tr("Flipped Horizontally");
                case ORIENTATION_ROT_180:
                    return QObject::tr("Rotated by 180 Degrees");
                case ORIENTATION_VFLIP:
                    return QObject::tr("Flipped Vertically");
                case ORIENTATION_ROT_90_HFLIP:
                    return QObject::tr("Flipped Horizontally and Rotated Left");
                case ORIENTATION_ROT_90:
                    return QObject::tr("Rotated Left");
                case ORIENTATION_ROT_90_VFLIP:
                    return QObject::tr("Flipped Vertically and Rotated Left");
                case ORIENTATION_ROT_270:
                    return QObject::tr("Rotated Right");
            }
        ...
        case MetadataInfo::WhiteBalanceColorTemperature:
            return QObject::tr("Temperature in Kelvin", "%1 K", value.toInt());
        ...
        case MetadataInfo::Longitude:
        {
            ...
            QString direction = (directionRef == 'W') ?
                                QObject::tr("For use in longitude coordinate", "West") : QObject::tr("For use in longitude coordinate", "East");
        ...
        case MetadataInfo::Latitude:
        {
            ...
            QString direction = (directionRef == 'N') ?
                                QObject::tr("For use in latitude coordinate", "North") : QObject::tr("For use in latitude coordinate", "South");
            return QString("%1%2%3%4%L5%6 %7").arg(degrees).arg(QChar(0xB0))
                                              .arg(minutes).arg(QChar(0x2032))
                                              .arg(seconds, 'f').arg(QChar(0x2033)).arg(direction);
        }
        case MetadataInfo::LatitudeNumber:
        {
            ...
            convertToUserPresentableNumbers(false, value.toDouble(), &degrees, &minutes, &seconds, &directionRef);
            QString direction = (directionRef == 'N') ?
                                QObject::tr("For use in latitude coordinate", "North") : QObject::tr("For use in latitude coordinate", "North");
            return QString("%1%2%3%4%L5%6 %7").arg(degrees).arg(QChar(0xB0))
                                              .arg(minutes).arg(QChar(0x2032))
                                              .arg(seconds, 'f').arg(QChar(0x2033)).arg(direction);
        }
        case MetadataInfo::Altitude:
        {
            ...
            return QObject::tr("Height in meters").arg("%L1m", meters);
        }
        ...
        case MetadataInfo::Title:
        {
            ...
            // Try "en-us"
            QLocale locale;
            QString spec = locale.languageToString(locale.language()).toLower() + '-' + locale.countryToString(locale.country()).toLower();
            if (map.contains(spec))
                return map[spec].toString();

            // Try "en-"
            QStringList keys = map.keys();
            QRegExp exp(locale.languageToString(locale.language()).toLower() + '-');
...
QMap<int, QString> DMetadata::possibleValuesForEnumField(MetadataInfo::Field field)
{
    ...
    switch (field)
    {
        ...
        case MetadataInfo::FlashMode:                        /// Int, bit mask from Exif
            ...
            map[0x1] = QObject::tr("Flash has been fired");
            map[0x40] = QObject::tr("Flash with red-eye reduction mode");
            //more: TODO?
            return map;
        default:
            qWarning() << "Unsupported field " << field << " in DMetadata::possibleValuesForEnumField" << endl;
            return map;
    }
                                      ...
                                      int& rating, QStringList& tagsPath) const
{
    ...
    {
        qDebug() << doc << endl;
        qDebug() << error << " :: row=" << row << " , col=" << col << endl;
        return false;
    }
