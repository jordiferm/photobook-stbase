...
void Charcoal::filterImage(void)
{
    ...
    {
       qWarning() << "No image data available!" << endl;
       return;
    }
    ...
    {
        qWarning() << "Image is smaller than radius!" << endl;
        return;
    }
    ...
    if(!kernel)
    {
        qWarning() << "Unable to allocate memory!" << endl;
        return;
    }
...
bool Charcoal::convolveImage(const unsigned int order, const double *kernel)
{
    ...
    if((kernelWidth % 2) == 0)
    {
        qWarning() << "Kernel width must be an odd number!" << endl;
        return(false);
    }
