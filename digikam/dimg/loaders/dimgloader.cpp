...
bool DImgLoader::checkExifWorkingColorSpace()
{
	...
	#else 
		//TODO: Profiles path configurable
// 		#if QT_VERSION >= 0x040400
// 			QString directory = QDesktopServices::storageLocation(QDesktopServices::DataLocation) + "/"; 
// 		#else 
// 			QString directory = QDir::home().absolutePath() + "/"; 
// 		#endif 
...
QByteArray DImgLoader::uniqueHash(const QString &filePath, const DImg &img, bool loadMetadata)
{
    ...
    if( qfile.open( QIODevice::Unbuffered | QIODevice::ReadOnly ) )
    {
        ...
        {
            QCrypt.addData( databuf, readlen );
            QCrypt.addData( size.setNum( qfile.size() ) );
            return QCrypt.result(); 
        }
        else
            return QByteArray();
    }
