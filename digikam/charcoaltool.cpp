...
namespace DigikamCharcoalImagesPlugin
{
			...
			: EditorToolThreaded(parent)
	{
		...
		QLabel *label1 = new QLabel(tr("Pencil size:"), m_gboxSettings->plainPage());
		...
		m_pencilInput->setWhatsThis(tr("Set here the charcoal pencil size used to simulate the drawing."));

		// -------------------------------------------------------------

		QLabel *label2 = new QLabel(tr("smoothing value of the pencil", "Smooth:"), m_gboxSettings->plainPage());
		...
		m_smoothInput->setWhatsThis(tr("This value controls the smoothing effect of the pencil "
		                               "under the canvas."));
	...
	{
		ImageIface iface(0, 0);
		iface.putOriginalImage(tr("Charcoal"), filter()->getTargetImage().bits());
	}
