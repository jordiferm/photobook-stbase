            ...
            : EditorToolThreaded(parent)
{
    ...
    QLabel *label1   = new QLabel(tr("Brush size:"), m_gboxSettings->plainPage());
    ...
    m_brushSizeInput->setWhatsThis(tr("Set here the brush size to use for simulating the oil painting."));

    // -------------------------------------------------------------

    QLabel *label2 = new QLabel(tr("value of smoothing effect", "Smooth:"), m_gboxSettings->plainPage());
    ...
    m_smoothInput->setWhatsThis(tr("This value controls the smoothing effect of the brush under the canvas."));
...
void OilPaintTool::putFinalData()
{
    ...
    iface.putOriginalImage(tr("Oil Paint"), filter()->getTargetImage().bits());
}
