...
namespace DigikamAdjustCurvesImagesPlugin
{
			...
			: EditorTool(parent)
	{
		...
		m_previewWidget = new ImageWidget("adjustcurves Tool", 0,
		                                  tr("This is the image's curve-adjustments preview. "
		...
		m_gboxSettings->histogramBox()->histogram()->setWhatsThis(tr("Here you can see the target preview "
		...
		m_curvesWidget = new CurvesWidget(256, 256, m_originalImage->bits(), m_originalImage->width(),
		                                  ...
		                                  curveBox);
		m_curvesWidget->setWhatsThis(tr("This is the curve drawing of the selected channel from "
		                                "original image"));
		...
		m_curveFree->setIcon(QPixmap((":/digikam/curvefree.png")));
		m_curveFree->setCheckable(true);
		m_curveFree->setToolTip(tr("Curve free mode"));
		m_curveFree->setWhatsThis(tr("With this button, you can draw your curve free-hand "
		                             "with the mouse."));
		m_curveType->addButton(m_curveFree, FreeDrawing);

		m_curveSmooth = new QToolButton(typeBox);
		m_curveSmooth->setIcon(QPixmap(":/digikam/curvemooth.png"));
		m_curveSmooth->setCheckable(true);
		m_curveSmooth->setToolTip(tr("Curve smooth mode"));
		m_curveSmooth->setWhatsThis(tr("With this button, you constrains the curve type "
		...
		m_resetButton->setToolTip(tr("Reset current channel curves' values."));
		m_resetButton->setWhatsThis(tr("If you press this button, all curves' values "
	...
	void AdjustCurvesTool::finalRendering()
	{
		...
		iface->putOriginalImage(tr("Adjust Curve"), desData);
	...
	{
	#if QT_VERSION >= 0x040400
		QUrl loadCurvesFile = QFileDialog::getOpenFileName(qApp->activeWindow(), tr("Select Gimp Curves File to Load"),
		...
		if (m_curvesWidget->curves()->loadCurvesFromGimpCurvesFile(loadCurvesFile) == false)
		{
			QMessageBox::critical(qApp->activeWindow(), tr("Error reading file"),
			                   tr("Cannot load from the Gimp curves text file."));
			return;
		}
	...
	{
		#if QT_VERSION >= 0x040400
			QUrl saveCurvesFile = QFileDialog::getSaveFileName(qApp->activeWindow(), tr("Gimp Curves File to Save") , 
											QDesktopServices::storageLocation(QDesktopServices::DocumentsLocation),QString("*"));
		#else
			QUrl saveCurvesFile = QFileDialog::getSaveFileName(qApp->activeWindow(), tr("Gimp Curves File to Save") , 
											QDir::homePath(),QString("*"));
		...
		if (m_curvesWidget->curves()->saveCurvesToGimpCurvesFile(saveCurvesFile) == false)
		{
			QMessageBox::critical(qApp->activeWindow(), tr("Error saving file"), 
			                   tr("Cannot save to the Gimp curves text file."));
			return;
		}
