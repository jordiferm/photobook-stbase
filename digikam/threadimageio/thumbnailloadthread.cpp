...
//    KIO::PreviewJob                *kdeJob;
	...
	ThumbnailLoadThread* ThumbnailLoadThread::defaultIconViewObject = 0;
	ThumbnailLoadThread* ThumbnailLoadThread::defaultObject = 0;
	ThumbnailLoadThread* ThumbnailLoadThread::defaultThumbBarObject = 0;
	...
	{
		if (!defaultIconViewObject)
			defaultIconViewObject = new ThumbnailLoadThread();
	...
	{
		if (!defaultObject)
			defaultObject = new ThumbnailLoadThread();
		return defaultObject;
	}

	ThumbnailLoadThread *ThumbnailLoadThread::defaultThumbBarThread()
	{
		if (!defaultThumbBarObject)
			defaultThumbBarObject = new ThumbnailLoadThread();
		return defaultThumbBarObject;
	}

	void ThumbnailLoadThread::cleanUp()
	{
		delete defaultIconViewObject;
		defaultIconViewObject = 0; 
		delete defaultObject;
		defaultObject = 0; 
		delete defaultThumbBarObject;
		defaultThumbBarObject = 0; 
	...
	bool ThumbnailLoadThread::checkSize(int size)
	{
		if (size <= 0)
		{
			qCritical() << "ThumbnailLoadThread::load: No thumbnail size specified. Refusing to load thumbnail."
			<< endl;
			return false;
		}
		else if (size > ThumbnailSize::Huge)
		{
			qCritical() << "ThumbnailLoadThread::load: Thumbnail size " << size
