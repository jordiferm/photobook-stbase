...
/*! 
	...
	Si l'usuari entra un _Settings correctes els guarda a la configuraci� de l'aplicaci�.
	S'ha de tenir en compte que es guarden settings diferents per a cada connectionName. 
		
	\example 
	// Crear� la base de dades per defecte.
	FSqlDatabase::start(); 
	// Per recuperar la base de dades ho farem:
	QSqlDatabase::database();
	
	// Crear� una base de dades amb el nom de conexi� "testdb"
	...
	while (!(_Settings.isValid() && NDatabase.isValid() && Connected) && GetAccepted)
	{
		...
		{	
			...
			else //Es que no estem conectats
			{
				...
				AskSettings = !WarnMessage.isEmpty(); //Si no hi ha queixa de la conexi� vol dir que no hem pas de demanar res de nou.
			}
		}
		int DResult;
		...
		if (_Settings.isValid() && NDatabase.isValid() && GetAccepted)
		{
			...
			// Ara ja tenim una configuraci� correcte i la base de dades existeix. Provem de conectar:
			if (TryConnect)
			{
...
bool FSqlDatabase::create(const FSqlDatabaseSettings& _Settings, QString& _WarningMessage, const QString& _SqlInitFile)
{
	...
	//SUPPORT PER MYSQL A LA BD. ( Alerta nom�s hi ha implementada la part de la connexi� no la part de creaci� i
	//gesti� de taules.
