...
public:
	...
	//! Estat en que est� la gesti� actual. No te gaire sentit si no utilitzem formularis modals.
...
public slots:
	...
	//! Aqu� index indica la fila on s'insertar� la nova fila.
...
signals: 
	...
	//! S'emet just despr�s de cridar insertRow();
	void afterInsertRow(int _Index); 
	//! Abans de cridar a edittRow()
	void beforeEditRow(int _Index, bool& _PreformOp); 
	//! Just despr�s d'insertar una row al model sense ser editada ni validada.
	void rowInserted(int _Index);
	//! S'emet just despr�s de cridar editRow(), si no hi ha RecDialog �s equivalent a rowInserted();
	void afterEditRow(int _Index); 
	void beforeRemoveRow(int _Index, bool& _PreformOp); 
	//! S'emet just despr�s de cridar removeRow();
