...
const int CONTEXTBAR_MARGIN = 20;
...
struct CheckItemDelegatePrivate
{
	/**
	...
	CheckItemDelegate* that;
	STThumbnailView* mView;
	QWidget* mContextBar;

	QToolButton* mAcceptButton;
	...
	bool hoverEventFilter(QHoverEvent* event, bool _ShowToolTips)
	{
...
//	#endif
	...
	void showToolTip(const QModelIndex& index)
	{
		...
		//FIXME: Calculate the height of the ToolTip.
		QPoint TipPos = mView->mapToGlobal(mView->geometry().topLeft()) - QPoint(0, 350);
		QToolTip::showText(TipPos , Text, mView);
	...
	void selectIndexUnderCursor()
	{
		if (mView->selectionModel()) //Defensive
			mView->selectionModel()->select(mIndexUnderCursor, QItemSelectionModel::Select | QItemSelectionModel::Rows);
	}

	void deselectIndexUnderCursor()
	{
		if (mView->selectionModel()) //Defensive
			mView->selectionModel()->select(mIndexUnderCursor, QItemSelectionModel::Deselect | QItemSelectionModel::Rows);
	}

	void toggleSelectionIndexUnderCursor()
	{
		if (mView->selectionModel()) //Defensive
			mView->selectionModel()->select(mIndexUnderCursor, QItemSelectionModel::Toggle | QItemSelectionModel::Rows);
	}

};


CheckItemDelegate::CheckItemDelegate(STThumbnailView* view)
		: QAbstractItemDelegate(view)
		, d(new CheckItemDelegatePrivate), ShowToolTips(false)
{
	d->that = this;
	d->mView = view;
	...
	d->mAcceptButton = d->createContextBarButton(":/st/accept.png");
	connect(d->mAcceptButton , SIGNAL(clicked()),
	        SLOT(slotAcceptButtonClicked()));

	d->mCancelButton = d->createContextBarButton(":/st/cancel.png");
	connect(d->mCancelButton, SIGNAL(clicked()),
	        SLOT(slotCancelButtonClicked()));

	QHBoxLayout* layout = new QHBoxLayout(d->mContextBar);
	layout->setMargin(2);
	layout->setSpacing(2);
	layout->addWidget(d->mAcceptButton);
	layout->addWidget(d->mCancelButton);

}


CheckItemDelegate::~CheckItemDelegate()
{
	delete d;
}
...
bool CheckItemDelegate::eventFilter(QObject*, QEvent* event)
{
	switch (event->type())
	{
		case QEvent::ToolTip:
		...
		case QEvent::HoverLeave:
			return d->hoverEventFilter(static_cast<QHoverEvent*>(event), ShowToolTips);
...
void CheckItemDelegate::setThumbnailSize(int value)
{
	d->mThumbnailSize = value;

	const int width = d->itemWidth();
	const int buttonWidth = d->mCancelButton->sizeHint().width();
...
void CheckItemDelegate::slotAcceptButtonClicked()
{
 	d->selectIndexUnderCursor();
 	emit acceptButtonClicked(d->mIndexUnderCursor);
}

void CheckItemDelegate::slotCancelButtonClicked()
{
 	d->deselectIndexUnderCursor();
 	emit cancelButtonClicked(d->mIndexUnderCursor);
}
