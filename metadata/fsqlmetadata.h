...
public: 
	...
	//! Retorna cert si la relaci� no cont� cap informaci�.
	bool isNull() const; 
};
...
public: 
	...
	QString xml() const;
	bool isNull() const {return IsNull;} //No est� carregada;
...
/**
Classe de MetaInformaci� sobre la base de dades.

Estructura de les metadades:
Cada taula es definir� amb un fitxer XML amb extensi� .mtd 
L'estructura del fitxer XML de les taules �s la seg�ent: 

<!DOCTYPE TMD>
<TMD revision="$Revision: 1.15 $" createrelations="false"> <!-- Revisi� controlada per CVS o manualment: $Revision: 1.15 $ -->
<!-- Parametres: (Opcional) createrelations per defecte = true, indica si es crearan les relacions -->
    ...
    <field> <!-- Definici� d'un camp de la taula -->
		<!-- Requerits -->
        ...
        <pk>true</pk> <!-- Si �s clau prim�ria. Per defecte false (Si hi ha varis camps llavors la clau �s composta. -->
        <serial>true</serial> <!-- Si cont� una seq��ncia. Per defecte false. -->
        ...
        <regexp>nnnn</regexp> <!-- Expressi� regular per a validar-lo -->
        <default>nnnn</default> <!-- Valor per defecte -->
        <editable>nnnn</editable> <!-- Si es pot editar -->
        <visible>nnnn</visible> <!-- Si es veur� per defecte en els viewers -->
		<!-- Definici� de relaci�. -->
        <relation>
	...
	<!-- S'executen a cada operaci�. -->
	<!-- Si s'indica el drivername nom�s s'executen en cas de que la base de dades utilitzi aquell driver. -->
	<!-- exclusive => Nom�s s'executar� aquest codi SQL, FLAM no far� res. -->
	<!-- S'executar� cada vegada que es crei de nou la taula despr�s de la creaci� d'aquesta. -->
...
INSERT INTO TABLE control (desc ) VALUES ('table tablename created !.');
	...
	<!-- S'executar� cada vegada que es crei de nou la taula abans de la creaci� d'aquesta. -->
	 <sqlbeforecreate exclusive="true" driver="drivername" ></sqlbeforecreate>
	<!-- S'executar� cada vegada que s'actualitzi la taula. Despr�s de l'actualitzaci� -->
	<!-- fromrevision => Nom�s s'executar� en cas de que actualitzem desde les revisions indicades. -->
	 <sqlafterupdate fromrevision="[&lt;,=,&lt;=,&gt;=,&gt;]nn.nn" driver="drivername"> </sqlafterupdate>
	<!-- S'executar� cada vegada que s'actualitzi la taula. Abans de l'actualitzaci� -->
	 <sqlbeforeupdate exclusive="true" fromrevision="[<,=,<=,>=,>]nn.nn" driver="drivername"> </sqlbeforeupdate>
	<!-- S'executar� cada vegada que es borri la taula. Despres del borrat de la taula. -->
	 <sqlafterdelete driver="drivername"> </sqlafterdelete>
	<!-- S'executar� cada vegada que es borri la taula. Abans del borrat de la taula. -->
